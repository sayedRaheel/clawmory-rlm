...
class Sub_RLM(RLM):
    ...
    def __init__(self, model: str = "gpt-5"):
        ...
        from clawmory_rlm.llm_client import OpenAIClient
    ...
    def completion(self, prompt) -> str:
        ...
        try:
            # Log sub-agent call
            print(f"[Sub_RLM.completion] Sub-agent called with model: {self.model}")
            if isinstance(prompt, str):
                print(f"[Sub_RLM.completion] Prompt length: {len(prompt)} characters")
            elif isinstance(prompt, (list, dict)):
                # Estimate length for messages
                total_len = sum(len(str(msg.get('content', ''))) for msg in (prompt if isinstance(prompt, list) else [prompt]))
                print(f"[Sub_RLM.completion] Messages total length: ~{total_len} characters")
            ...
            print(f"[Sub_RLM.completion] Response received, length: {len(response)} characters")
            return response
                
        except Exception as e:
            error_msg = f"Error making LLM query: {str(e)}"
            print(f"[Sub_RLM.completion] ERROR: {error_msg}")
            return error_msg
...
class REPLEnv:
    def __init__(
        ...
        def llm_query(prompt: str) -> str:
            ...
            # Log that sub-agent is being called
            print(f"\n[SUB-AGENT CALL] Triggered with prompt length: {len(prompt)} characters")
            try:
                result = self.sub_rlm.completion(prompt)
                print(f"[SUB-AGENT CALL] Completed successfully, result length: {len(result)} characters")
                return result
            except Exception as e:
                print(f"[SUB-AGENT CALL] Error: {str(e)}")
                raise
        ...
        try:
            ...
            with open(log_path, 'a') as f:
                f.write(json.dumps({
                    ...
                    'runId': 'init',
                    'hypothesisId': 'A',
                    'location': 'repl.py:193',
                    'message': 'llm_query function added to REPL globals',
                    'data': {
                        'llm_query_available': 'llm_query' in self.globals,
                        'sub_rlm_initialized': hasattr(self, 'sub_rlm'),
                        'sub_rlm_model': getattr(self.sub_rlm, 'model', None) if hasattr(self, 'sub_rlm') else None
                    },
